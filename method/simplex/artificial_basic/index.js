...
function solveWithArtificialBasicMethod(task)
{
	...
	for(var i = 0; i < task.equationsCount; i++)
		if(task.freeMembers[i].isNegative())
		{
	...
	var newTargetFunctionCoefficients = new Array(newCoefficients[0].length).fill(BaseType.getZero());

	for(var i = 0; i < task.equationsCount; i++)
		newTargetFunctionCoefficients[i] = BaseType.getOne().neg();
	...
	var splx2 = new SimplexTable(artificialBasicTask);
...
module.exports = {solveWithArtificialBasicMethod};
